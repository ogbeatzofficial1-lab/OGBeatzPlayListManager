...
export default function VideoGenerationModal({ track, playlist, onClose }: VideoGenerationModalProps) {
  const { addPromoVideo, tracks: allTracks, pullLyrics } = useMediaStore();
  ...
  const [rawLyricsInput, setRawLyricsInput] = useState<string>('');
  const [showAIInput, setShowAIInput] = useState<boolean>(false);

  const handlePullLyrics = async (customText?: string) => {
    if (!activeTrack) return;
    setIsPullingLyrics(true);
    try {
      const results = await pullLyrics(activeTrack, customText);
      if (results && results.length > 0) {
        const lyricsWithIds = results.map((lyric: any, idx: number) => ({
          id: lyric.id || `ai-lyric-${idx}-${Math.random().toString()}`,
          text: lyric.text,
          startTime: lyric.startTime,
          endTime: lyric.endTime,
        }));
        setLyrics(lyricsWithIds);
        setShowAIInput(false);
      }
    } catch (e) {
      console.error("Error pulling lyrics:", e);
    } finally {
                        ...
                        <div className="flex items-center justify-between pb-1 border-b border-zinc-900">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className={`text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5 px-3 py-1.5 rounded-xl transition-all border ${
                                showAIInput 
                                ? 'bg-orange-500/10 border-orange-500/30 text-orange-400' 
                                : 'bg-zinc-900 hover:bg-zinc-800 border-zinc-800 text-zinc-100 hover:text-orange-400'
                              }`}
                              title="Sync lyrics automatically or paste official lyrics"
                            >
                              <Sparkles className="w-3 h-3 text-orange-400" />
                              {showAIInput ? 'Close AI Workspace' : 'AI Align & Sync'}
                            </button>
                            <button
                              type="button"
                            ...
                            >
                              <Plus className="w-3 h-3 fill-current" /> Add Lyric Line
                            </button>
                          </div>
                        </div>

                        {/* Collapsible Paste Lyrics & Sync Studio */}
                        <AnimatePresence>
                          {showAIInput && (
                            <motion.div 
                              ...
                              className="bg-zinc-900/45 border border-zinc-900 rounded-3xl p-5 space-y-4 overflow-hidden"
                            >
                              <div className="flex items-center justify-between">
                                <h5 className="text-[9px] font-black uppercase tracking-wider text-orange-400 flex items-center gap-1.5">
                                  <Sparkles className="w-3.5 h-3.5" /> AI LYRIC ALIGNMENT WORKSPACE
                                </h5>
                                <button
                                  type="button"
                              ...
                              <p className="text-[9px] text-zinc-400 leading-relaxed">
                                Avoid AI fake lyrics by pasting your exact official lyrics below! Gemini AI will preserve your words perfectly, break them down into shorttimed subtitle segments, and align them smoothly across the track timeline.
                              </p>
                              
                              <textarea
                                value={rawLyricsInput}
                                onChange={(e) => setRawLyricsInput(e.target.value)}
                                placeholder="Paste your official lyrics here... (e.g. &#10;[Verse 1]&#10;Yeah, back in the studio working on the mix...)"
                                rows={5}
                                className="w-full bg-zinc-950 border border-zinc-900 rounded-2xl p-4 text-xs font-sans text-white focus:border-orange-500 outline-none leading-relaxed"
                              />

                              <div className="flex flex-col sm:flex-row gap-3">
                                <button
                                  type="button"
                                  onClick={() => handlePullLyrics(rawLyricsInput)}
                                  disabled={isPullingLyrics || !rawLyricsInput.trim()}
                                  className="flex-1 py-3 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-black font-black uppercase tracking-widest text-[9px] rounded-xl flex items-center justify-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                                  {isPullingLyrics ? (
                                    <>
                                ...
                                <button
                                  ...
                                  disabled={isPullingLyrics}
                                  className="py-3 px-4 bg-zinc-950 hover:bg-zinc-900 border border-zinc-900 text-zinc-300 font-bold uppercase tracking-widest text-[9px] rounded-xl disabled:opacity-50 flex items-center justify-center gap-1.5"
                                  title="Generate standard song structure markers like [Intro], [Drop], [Chorus] spaced across the timeline."
                                >
                                  <Clock className="w-3.5 h-3.5 text-zinc-500" />
                                  Use Instrumental Structure
                                </button>
                              </div>
                            </motion.div>
                          )}
